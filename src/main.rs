...
impl Note {
    fn new(reader: &mut Cursor<Vec<u8>>) -> Note {
        ...
        let desc = desc_.clone();

        let note_type = NoteType::new(type_, desc_);
...
impl NoteType {
    fn new(value: u32, data: Vec<u8>) -> NoteType {
        use NoteType::*;
        ...
        match value {
            ...
            2 => GnuHwCap(data),
            3 => GnuBuildID(format!("{:x?}", data)),
            4 => GnuGoldVersion(format!("{:x?}", data)),
            5 => GnuProperty(data),
            _ => Unknown(value, data),
        }
    }
}
